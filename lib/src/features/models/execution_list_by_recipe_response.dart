...
class ExecutionListByRecipeResponse {
  ...
  Map<String, dynamic> toJson() => {
    'completedExecutions' :  completedExecutions.map((execution) => execution.toProto3Json()).toList(),
    'pendingExecutions' : pendingExecutions.map((execution) => execution.toProto3Json()).toList()
  };
  ...
  factory ExecutionListByRecipeResponse.fromJson(Map<String, dynamic> json) {
    return ExecutionListByRecipeResponse(pendingExecutions: List.from(json['pendingExecutions']).map((execution) => Execution.create()..mergeFromProto3Json(execution)).toList(),
        completedExecutions: List.from(json['completedExecutions']).map((execution) => Execution.create()..mergeFromProto3Json(execution)).toList());
  }
