...
class GetExecutionByIdHandler implements IPCHandler{
  ...
  void handler(SDKIPCResponse<dynamic> response) {
    final defaultResponse = SDKIPCResponse<Execution>(success: response.success, action: response.action, data: Execution()..createEmptyInstance(), error: response.error, errorCode: response.errorCode);
    try {
      if (response.success) {
