...
class GetRecipeHandler implements IPCHandler{
  ...
  void handler(SDKIPCResponse<dynamic> response) {
    final defaultResponse = SDKIPCResponse<Recipe>(success: response.success, action: response.action, data: Recipe()..createEmptyInstance(), error: response.error, errorCode: response.errorCode);
    try {
      if (response.success) {
        defaultResponse.data =  Recipe.create()..mergeFromProto3Json(response.data);
      }
